...
from configuration_file import NewProductUrlId, CrawlProductUrlId, NewProductUrl, PoaId, DbNum, PoaUuid, ThreadNum
        ...
        try:
            ...
            if not rds_12.exists_key(poa_id_key):
                ...
                try:
                    ...
                    traceback.print_exc()
                print(product_url)
                rds_12.set_hash(poa_id_key, {'uuid': _uuid})
                rds.remove_member(CrawlProductUrlId, url_id)
            else:
                rds.remove_member(CrawlProductUrlId, url_id)
                rds.delete_key(poa_key)
...
def dx_product_main(que):
    try:
        # 除去下面这部分，和dx_popular_product.py一样
