...
from configuration_file import (
    NewProductUrlId, CrawlProductUrlId, NewProductUrl, PoaId, DbNum, PoaUuid, ThreadNum)
...
class ConsumerThread(Thread):
    ...
    def parse_html(self, html, url_id, p_url):
        try:
            poa_key = PoaId + url_id
            poa_id_key = PoaUuid + str(url_id)
            if not rds.exists_key(poa_id_key):
                ...
                if len(category_url) > 0:
                    ...
                    category_url = ''
                    category_id = ''
                #product_url = 'http://www.dx.com/%s' % url_id
                product_url = p_url
                ...
                if not rds.exists_key(poa_key):
                    ...
                    if len(product_attr) > 0:
                        ...
                        other_poa_list = [attr['Sku'] for attr in attr_lst if int(attr['Sku']) != int(url_id)]
                        version_urls = ','.join(
                            ['http://www.dx.com/%s' % other_poa for other_poa in other_poa_list if int(other_poa) != int(url_id)])
...
def dx_product_main(que):
    try:
        if rds.exists_key(NewProductUrl):
            new_product_url = rds.get_all_members(NewProductUrl)
            for npu in new_product_url:
                if isinstance(npu, bytes):
                    npu = npu.decode('utf-8')
                npu_id = re.findall(r'-(\d+)[%,#]', npu)
                if npu_id:
                    npu_id = npu_id[0]
                    poa_id_key = PoaUuid + str(npu_id)
                    if not rds.exists_key(poa_id_key):
                        rds.add_set(NewProductUrlId, npu_id)
            #rds_0.delete_key('new_product_url')
