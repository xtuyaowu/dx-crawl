...
class DxStore:
    ...
    def select_track_by_status(self, status):
        sql = "select scgst_id, scgs_products_id from scb_crawler_global_track where scgst_status=%s"
        cur = self.conn.cursor()
        cur.execute(sql, status)
        return cur.fetchall()

    def update_track_uuid_status(self, _uuid, status, _id):
        sql = "update scb_crawler_global_track set scgs_uuid=%s, scgst_status=%s where scgst_id=%s"
        cur = self.conn.cursor()
        cur.execute(sql, (_uuid, status, _id))
        self.conn.commit()
